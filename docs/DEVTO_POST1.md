---
title: "DNS has no `git`. I built one."
published: false
description: A weekend prototype against a 40-year-old gap in how DNS treats its own state.
...
`zonegitd` is not trying to replace BIND or Knot or NSD. Those are decades-hardened pieces of infrastructure and ripping them out is nobody's day-one move. The intended deployment shape is **zonegit as the control plane, your existing authoritative server as a downstream secondary** over standard AXFR/IXFR. The reference server exists so the demo is honest end-to-end and so greenfield deployments have a path. It is not the adoption vector.
